...
class DimPoint:
    ...
    def outer_line_length(self, other) -> float:
        ...
        The x or y coordinated is adjust by getting the inside dimension of the DimPoint object and assigning
...
class DimPath:
    ...
    def set_wrap(self) -> None:
        ...
        # set the last point's next to the first point and the first point's prev to the last point
...
class Base:
    ...
    def calc_min(self, tot_len: float) -> tuple[int, int, float]:
        """
        ...
        # TODO: is the use case of 1 finger and 0 spaces valid? YES I THINK IT IS
    ...
    def normalize_paths(self):
        # Normalizes paths by copying and wrapping points
    ...
    def proc_walls(self):
        # print("-" * 100)
        walls_horz: list[Wall] = []
        walls_vert: list[Wall] = []

        # loop over the exterior walls (auto generated by create_path_walls()) and the
        #   interior walls (manually created by calling add_wall())
        for index_wall in self.index_walls:
            ...
            wall: Wall = Wall(start_avg_pt, end_avg_pt, index_wall.wall_type, index_wall.start_pt.dim_point)
